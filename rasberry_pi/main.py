...
stairs = 0 #階段上
flag = False #mainのループ
stairs_time = 3 #階段から床への判断回数

setting_distance = 5 #稼働までの距離センサー１

#壁の対処パラメータ
out_distance = 15 #壁を検知する距離(cm)
go_back_time = 5 #壁に当たってから後退する時間(s)
turn_time = 5 #壁に当たった時の回避するための回転時間(s)左折
turn_time2 = 5 #壁に当たった時の回避するための回転時間(s)右折
wall_distance = 25 #壁か階段の距離(cm)(上の距離)(小さければ壁、大きければ階段)

#階段上り
step_turn_time = 0.5 #階段の回転(s)
    ...
    while flag:
        ...
        if distance["right_front"] <= out_distance and distance["right_front"] != -1 and sett == 0 \
            ...
            if distance["top"] >= wall_distance and distance["top"] != -1:#階段
                ...
                while True:
                    ...
                    print("上階段対処distance_sub1:distance_sub2:distance1;" + str(distance_sub1) + ":" + str(distance_sub2) + ":" + str(distance1))
            ...
            if distance["top"] < wall_distance and distance["top"] != -1 and sett == 0:#壁
                ...
                GPIO.output(motor4_pin, False)
                distance1 = distance["left_front"] - distance["right_front"]
                print("壁対処distance1:" + str(distance1))
                                        ...
                                        if distance_sub2 <= under_out_distance and distance_sub2 != -1:#右
                                            ...
                                            while ff:
                                                ...
                                                if distance_sub２ >= under_out_distance and distance_sub２ != -1:#右
                                                    ff = False
                                                    print("下階段対処dis_time:" + str(dis_time))
        ...
        if stairs == 1:
            ...
            while True:
                ...
                print("階段通過中;distance_stairs1:distance_stairs2;" + str(distance_stairs1) + ":" + str(distance_stairs2))
...
def controller():   
    ...
    while True:
        ...
        print("setting_d,setting_d2,setting_d3:" + str(setting_d) + "," + str(setting_d2) + "," + str(setting_d3))

        if setting_d <= setting_distance and setting_d != -1:
            print("=========プログラム開始==========")
            ...
            while True:
                ...
                if ii >= 10:
                    ...
                    print("=========プログラム停止(シャットダウン)==========")
                    log_damba.memo_write("=========プログラム停止(シャットダウン)==========")
                    flag = False
                    ...
                    print("~~~~~~~~~~~システム停止(シャットダウン)~~~~~~~~~~~")
                    log_damba.memo_write("~~~~~~~~~~~システム停止(シャットダウン)~~~~~~~~~~~")
                    os.system("sudo shutdown -h now")
                time.sleep(0.1)

        if setting_d2 <= setting_distance and setting_d3 <= setting_distance and setting_d2 != -1 and setting_d3 != -1:
            print("=========プログラム停止==========")
